...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Android version should be Marshmallow and above
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            fingerprintManager = (FingerprintManager) getSystemService(FINGERPRINT_SERVICE);
            keyguardManager = (KeyguardManager) getSystemService(KEYGUARD_SERVICE);

            //Check if device has fingerprint scanner, requires fingerprint permission
            if (!fingerprintManager.isHardwareDetected()) {
                mParaLabel.setText("Fingerprint scanner not detected in device!");
            } else if (ContextCompat.checkSelfPermission(this, Manifest.permission.USE_FINGERPRINT )!= PackageManager.PERMISSION_GRANTED) {
                mParaLabel.setText("Please grant permission to use fingerprint scanner");
            } //check if lock screen has at least one type of lock
            else if (!keyguardManager.isKeyguardSecure()) {
                mParaLabel.setText("Add a lock to your screen in Settings");
            } //check if there is a registered fingerprint
